...
| D-05 | ¿`sin_reparacion` es terminal o puede pasar a `entregado`?                                   | La máquina de estados final la define la operación real del taller                                                                                                         | ⏳ Abierta                                                                          | —          |
...
3. **D-04/D-05**: reglas operativas del taller (validar con un caso real: ¿qué ve el cliente en seguimiento? ¿qué pasa si el cliente rechaza el presupuesto?).
