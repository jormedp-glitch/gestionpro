...
> Detectados durante la ronda de pruebas manuales en producción; reparados en batch el 2026-09-14 (#80–#82). La gestión de usuarios desde la app se resolvió en FASE 6 (ver sección FASE 6 y TC-ADMIN-04 en `docs/casos-de-prueba-manuales.md`).

- [x] **Gestión de usuarios desde la app (FASE 6)**: alta/invitación de usuarios, membresías por negocio (asignar/quitar) y roles — hoy se hace a mano en Supabase (dashboard/SQL). Origen: TC-ADMIN-04 (segundo usuario creado por SQL). — **Resuelto en FASE 6** (`fase6-gestion-usuarios`, 2026-09-21): ver sección FASE 6.
- [x] **Cerrar sesión desde el panel `/`**: `logout()` ya existe (`lib/auth/actions.ts`) pero el botón "Salir" solo está en `NegocioShell` (dentro de `/{slug}`). Agregar el botón al panel de administración. Relacionado: TC-AUTH-05. — **Reparado en #80**.
- [x] **Link "Reparaciones" visible en rubros que no son de reparaciones**: `NegocioShell` muestra "🔧 Reparaciones" en el branch de rubros no-`servicio_tecnico` (`features/admin/components/NegocioShell.tsx:128-133`); debe aparecer solo en rubros de reparaciones. Ojo: la condición usa `rubro === "servicio_tecnico"` (el seed viejo usa el valor `reparaciones`). — **Reparado en #81**.
- [x] **Estados de cliente por vencimiento nunca se calculan (y las "⚡ Alertas de cobro" no aparecen)**: `vence_pronto`/`vencido` existen en la UI pero nada los setea — `agregarCliente` inserta `activo` y `pagarCliente` vuelve a `activo` sin tocar `vence`; las alertas filtran `estado !== "activo"` (`features/admin/components/DashboardResumen.tsx:39`). Evidencia: clientes con `vence` 06/04 y 07/04/2026 (meses vencidos) siguen "Activo" → la tarjeta "⚡ Alertas de cobro" nunca aparece. Resuelto: estado derivado del `vence` al leer (`lib/domain/cuotas.ts`: vencido / vence_pronto ≤ 7 días / activo) y `✓ Pagó` extiende el `vence` al próximo período. — **Reparado en #82**.

### Hallazgo del Security Advisor de Supabase (2026-09-13 → cerrado 2026-09-15)

> Reporte automático del Security Advisor (email del 2026-09-13, proyecto `qjawdjzaokffhiqnixcx`, main/PRODUCTION): `RLS Disabled in Public` sobre la tabla de auditoría del backfill de `numero_orden`. Registrado en este plan con su cierre (quedó pendiente de la sesión del 2026-09-15).

- [x] **`public._fase3_duplicates_log` con RLS deshabilitado y grants a `anon`/`authenticated`**: 0003 creó la tabla dentro de un bloque `DO $$` pero **fuera** del bloque de RLS de 0001 y sin revocar los grants por defecto de Supabase (`anon`/`authenticated` nacen con `ALL` sobre tablas nuevas de `public`) → la anon key (pública por diseño, viaja en el bundle del cliente) permitía leer/editar/borrar el log vía PostgREST. Sin datos de negocio en la tabla y sin acceso de runtime, pero el advisor bloqueaba con 1 error. La tabla hermana `negocio_orden_contadores` no aparecía porque 0003 sí le revocó los grants (líneas 103-104). — **Cerrado**: migración aditiva e idempotente `supabase/migrations/0004_fase3_duplicates_log_rls.sql` (`enable row level security` + `revoke all ... from anon, authenticated`; sin policies a propósito: solo auditoría manual, `service_role`/postgres siguen leyendo) → issue #83 → PR #84 mergeado. Verificado en producción (SQL editor, 2026-09-15): `relrowsecurity = true` y 0 grants para anon/authenticated.
...
- **Portal público del alumno**: `/[slug]/portal/[token]` sin sesión (RPCs `security definer` de `0007_fase7_portal.sql`; token UUID como capacidad, sin oráculo) + gate en `proxy.ts` (`esRutaPublica`).
- **Migración de los datos reales**: `scripts/migracion-coachflow-auth.sql` (profes → `auth.users` + `negocios` + `negocio_miembros`, D1/D2), `migracion-coachflow.sql` + `migracion-coachflow-actividad.sql` (transaccionales, idempotentes por `_fase7_migracion_map`), `verify-coachflow.sql` (PASS/FAIL) y `rollback-coachflow.sql`.
- **Retiro**: `supabase/migrations/0008_fase7_drop_cf.sql` (drop de las 11 `cf_*` + tablas internas, con gate post-verify y backup).

**Decisiones clave** (spec D1–D6 + design AD-1…AD-7):

| Decisión      | Elegida                                                                                |
| ------------- | -------------------------------------------------------------------------------------- |
| Cobros        | Tabla `cobros` en el núcleo (todos los rubros), no `gym_pagos` (AD-1)                  |
| Portal        | `portal_token uuid` como capacidad; `codigo_acceso` legacy queda como dato (AD-2, R17) |
| Biblioteca    | Catálogo global (`negocio_id is null`) de solo lectura para `authenticated` (AD-3)     |
| Rutina activa | Una por alumno, garantizada por unique index parcial en la base (AD-4), no en la app   |
| Completados   | Unique `(cliente_id, rutina_ejercicio_id, fecha)` + upsert idempotente (AD-5)          |
| Migración     | SQL transaccional e idempotente con tabla map, author-only, sin service key (AD-7)     |
| Profes        | Contraseña actual como inicial (D1); sin email → `<slug>@migrado.invalid` (D2)         |
| App CoachFlow | Se apaga y redirige a GestiónPro (D4)                                                  |
...
#### Rollout final — runbook (lo ejecuta el maintainer)

> Orden obligatorio, con rol privilegiado (postgres / service_role), la app de FASE 7 ya mergeada/desplegada y `psql -1` (transacción única, D-13). No continuar si un paso no cierra OK. Si algo falla ANTES del paso 9 → `rollback-coachflow.sql` (borra los destinos por map; `cf_*` intactas) y reintentar. DESPUÉS del paso 9 el rollback es restaurar el backup del paso 8.

| #   | Paso                       | Comando / acción                                                                                                 | OK si                                                   |
| --- | -------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| 1   | Backup pre                 | `pg_dump "$DATABASE_URL" --format=custom --file=backup-pre-fase7-$(date +%Y%m%d).dump` + `pg_restore --list`     | el dump lista contenido y no está vacío                 |
| 2   | Esquema + RPCs             | `psql -1 … -f supabase/migrations/0006_fase7_gym_cobros.sql` y `0007_fase7_portal.sql`                           | sin errores                                             |
| 3   | RLS de lo nuevo            | `psql … -v tenant=… -v owner=… -v other=… -f scripts/verify-rls.sql`                                             | `verify-rls: TODAS LAS COMPROBACIONES OK`               |
| 4   | Auth con UN profe          | `psql -f scripts/migracion-coachflow-auth.sql` con el filtro de 1 profe (ver VALIDACIÓN del header) + login real | el profe entra y ve su negocio (escenario 14)           |
| 5   | Auth del resto             | re-ejecutar sin el filtro                                                                                        | reporte de migrados/reusados                            |
| 6   | Datos                      | `psql -f scripts/migracion-coachflow.sql` y `-f scripts/migracion-coachflow-actividad.sql`                       | reportes de counts; re-ejecutable sin duplicar          |
| 7   | Verificación               | `psql -f scripts/verify-coachflow.sql`                                                                           | exit 0 (PASS)                                           |
| 8   | Backup post                | igual que el paso 1 (`backup-post-fase7-…`)                                                                      | verificado (es el punto de restore del retiro)          |
...
# alternativa contra un cluster local con 0001..0008 aplicadas:
supabase gen types typescript --db-url "postgresql://postgres@127.0.0.1:54322/postgres" --schema public > types/database.types.ts
```

### FASE 6 — Producto y automatización (post-MVP, por slices) — riesgo: Medio — ✅ gestión de usuarios (2026-09-21)
