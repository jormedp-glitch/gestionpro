...
export default function NegocioPage({
  ...
  const color =
    negocio.rubro === "peluqueria"
      ? "#A78BFA"
      : negocio.rubro === "veterinaria"
        ? "#34D399"
  ...
  const agregarCliente = async () => {
    if (!modalData.nombre || !modalData.plan || !modalData.cuota)
    ...
    await supabase.from("clientes").insert([
      {
        ...
        plan: modalData.plan,
        cuota: Number(modalData.cuota),
        vence: modalData.vence || hoy,
        estado: "activo",
      },
    ]);
    supabase
  ...
  const agregarTurno = async () => {
    ...
    await supabase.from("turnos").insert([
      {
        negocio_id: negocio.id,
        cliente_nombre: modalData.clienteNombre,
        telefono: modalData.telefono || "",
        servicio: modalData.servicio,
        fecha: modalData.fecha,
        hora: modalData.hora,
        duracion: Number(modalData.duracion || 60),
        estado: "confirmado",
    ...
    if (modalData.telefono) {
      ...
      window.open(
        `https://wa.me/54${modalData.telefono.replace(/\D/g, "")}?text=${encodeURIComponent(msg)}`,
        "_blank",
      );
    }
    setModal(null);
  ...
  const agregarGasto = async () => {
    if (!gastoForm.desc || !gastoForm.monto)
      return showToast("Completá el gasto");
    await supabase.from("gastos").insert([
      {
        negocio_id: negocio.id,
        descripcion: gastoForm.desc,
    ...
    supabase
      ...
      .eq("negocio_id", negocio.id)
      .then(({ data }) => setGastos(data || []));
    setGastoForm({ desc: "", monto: "", fecha: hoy });
          ...
          {negocio.rubro === "servicio_tecnico" ? (
            <>
              <button
                ...
                style={{
                  background: vista === "dashboard" ? color + "20" : "none",
                  color: vista === "dashboard" ? color : "#666",
                  border: "none",
                  cursor: "pointer",
                  padding: ".5rem .9rem",
                  borderRadius: "8px",
              ...
              <button
                ...
                style={{
                  background: vista === "gastos" ? color + "20" : "none",
                  color: vista === "gastos" ? color : "#666",
                  border: "none",
                  cursor: "pointer",
                  padding: ".5rem .9rem",
                  borderRadius: "8px",
            ...
            <>
              {[
                ...
                <button
                  ...
                  onClick={() => setVista(v)}
                  style={{
                    background: vista === v ? color + "20" : "none",
                    color: vista === v ? color : "#666",
                    border: "none",
                    cursor: "pointer",
                    padding: ".5rem .9rem",
                    borderRadius: "8px",
      ...
      >
        {vista === "dashboard" && (
          <div>
            ...
            >
              {[
                [activos, "✅", "Activos", "#34D399"],
                [turnosHoy.length, "📅", "Turnos hoy", color],
                [formatARS(ingresoMes), "💰", "Ingreso mes", "#60A5FA"],
                [
                  formatARS(ingresoMes - gastosMes),
                  "📊",
                  "Neto mes",
                  ingresoMes - gastosMes >= 0 ? "#34D399" : "#F87171",
                ],
              ].map(([v, i, l, c]: any, idx) => (
                <div
                  key={idx}
            ...
            >
              ...
              {turnosHoy.map((t) => (
                ...
                >
                  <div>
                    <span
                      style={{ color, fontWeight: 700, marginRight: ".75rem" }}
                    >
                      {t.hora}
                    </span>
                      ...
                      >
                        ...
                        {c.telefono && (
                          <button
                            onClick={() =>
                              window.open(
                                `https://wa.me/54${c.telefono.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${c.nombre}, tu cuota en *${negocio.nombre}* está pendiente. ¿Cuándo podés pasar? 😊`)}`,
                                "_blank",
                              )
                            }
                            style={{
                    ...
                    <div style={{ display: "flex", gap: ".5rem" }}>
                      {t.telefono && (
                        <button
                          onClick={() => {
                            ...
                            const tot = h * 60 + m + 30;
                            const hr = `${String(Math.floor(tot / 60)).padStart(2, "0")}:${String(tot % 60).padStart(2, "0")}`;
                            window.open(
                              `https://wa.me/54${t.telefono.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${t.cliente_nombre}, tu turno de ${t.servicio} tiene una demora de 30 min. Nuevo horario: ${hr}hs. Disculpá! 🙏`)}`,
                              "_blank",
                            );
                          }}
                          style={{
                      ...
                      {t.telefono && (
                        <button
                          onClick={() =>
                            window.open(
                              `https://wa.me/54${t.telefono.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${t.cliente_nombre} 🌟 Te recordamos tu turno de ${t.servicio} hoy a las ${t.hora}hs en *${negocio.nombre}*. ¡Te esperamos!`)}`,
                              "_blank",
                            )
                          }
                          style={{
                  ...
                  <div style={{ display: "flex", gap: ".35rem" }}>
                    ...
                    {c.telefono && (
                      <button
                        onClick={() =>
                          window.open(
                            `https://wa.me/54${c.telefono.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${c.nombre}, te contactamos desde *${negocio.nombre}*. Tu cuota está pendiente. ¿Cuándo podés pasar? 😊`)}`,
                            "_blank",
                          )
                        }
                        style={{
            ...
            >
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setGastoForm({ ...gastoForm, desc: e.target.value })
                  }
                />
                <input
                  style={inp}
                  ...
                  >
                    ...
                    <button
                      onClick={async () => {
                        ...
                        setGastos(gastos.filter((x) => x.id !== g.id));
                        showToast("Eliminado");
                      }}
                      style={{
                        background: "#F8717115",
          ...
          >
            ...
            >
              <input
                ...
                value={modalData.clienteNombre || ""}
                onChange={(e) =>
                  setModalData({ ...modalData, clienteNombre: e.target.value })
                }
              />
              <input
                style={inp}
              ...
              <input
                ...
                onChange={(e) =>
                  setModalData({ ...modalData, notas: e.target.value })
                }
              />
            </div>
            <div
          ...
          >
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setModalData({ ...modalData, plan: e.target.value })
                }
              />
              <input
                style={inp}
                ...
                value={modalData.cuota || ""}
                onChange={(e) =>
                  setModalData({ ...modalData, cuota: e.target.value })
                }
              />
              <input
                style={inp}
                type="date"
                value={modalData.vence || ""}
                onChange={(e) =>
                  setModalData({ ...modalData, vence: e.target.value })
                }
              />
            </div>
            <div
